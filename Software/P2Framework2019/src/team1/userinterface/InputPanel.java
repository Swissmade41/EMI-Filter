...
public class InputPanel extends JPanel {
	...
	private static final double DEFAULT_VALUES[] = { 0.5e-3, 7.92e3, 6.46e-12, 9.2e-6, 11.3e-3, 146.7e-9, 15.2e-9,
			47.4e-3, 146.7e-9, 15.2e-9, 47.4e-3, 3.85e-9, 8e-9, 246e-3 };

	private String[] parameter = { "<html>L<sub>0</sub></html>", "<html>C<sub>X1</sub></html>",
			"<html>C<sub>X2</sub></html>", "<html>C<sub>Y1</sub></html>" };

	private String[][] subParameter = {
			{ "<html>L<sub>0</sub></html>", "<html>R<sub>p</sub></html>", "<html>C<sub>p</sub></html>",
					"<html>L<sub>r</sub></html>", "<html>R<sub>w</sub></html>" },
	...
	private String[][] units = { { "H", "\u03A9", "F", "H", "\u03A9" }, { "F", "H", "\u03A9" }, { "F", "H", "\u03A9" },
			{ "F", "H", "\u03A9" } };
	...
	public InputPanel(Controller controller) {
		...
		add(informationPanel, new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
		...
		for (int n = 0; n < parameter.length; n++) {
			...
			add(paramter_Panel, new GridBagConstraints(n + 1, 1, 1, 1, 1.0, 1.0, GridBagConstraints.CENTER,
					GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));
		}
	}
	 ...
	 * 
	 * @return User input parameter value
	 */
	...
	public void updateInputPanel(double[] textfieldValues, double[] parameterValues) {
		...
		for (int n = 0; n < parameter.length; n++) {
			for (int m = 0; m < subParameter[n].length; m++) {
				...
				int sliderValue = (int) (((parameterValues[s32_tmpCounter] - textfieldValues[s32_tmpCounter]) * 100)
						/ textfieldValues[s32_tmpCounter]); // calculate sliderposition
...
/**
 * Input sub panel with parameter label, text field, slider, effective value
 * label with handling the listeners
 *
 */
class InputSubPanel extends JPanel implements ChangeListener, ActionListener {
	...
	/**
	 * Builds the sub panel with parameter label, text field, slider, effective
	 * value label
	 * 
	 * @param controller   Controller object
	 * @param subParameter Name of the sub parameter
	 */
	public InputSubPanel(Controller controller, String subParameter, String unit) {
		...
		add(label_subParameter, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 10, 10));
		add(textfield_ParameterValues, new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.HORIZONTAL, new Insets(5, 5, 5, 5), 10, 10));
		add(slider_Parameter, new GridBagConstraints(0, 2, 1, 1, 1.0, 1.0, GridBagConstraints.CENTER,
				GridBagConstraints.VERTICAL, new Insets(5, 5, 5, 5), 10, 50));
		add(label_ParameterValue, new GridBagConstraints(0, 3, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 10, 10));
	 ...
	 * 
	 * @param value New value of the user parameter value
	 */
	 ...
	 * Setter of the effective parameter value
	 * 
	 * @param value New value of the effective parameter value
	 */
	...
	/**
	 * Refresh text in textfield and the effective parameter label and update the
	 * parameter values
	 */
	 ...
	 * 
	 * @param n The new value of the slider
	 */
	...
	/**
	 * Textfield text changed; read the new content in the textfield and update the
	 * label of effective parameter value and the parameter values
	 */
...
class InformationPanel extends JPanel {
	...
	/**
	 * set null layout; modifiability not needed add percent labels
	 */
	public InformationPanel() {
		...
		tmp_n += add;
		add(new JLabel("20%")).setBounds(30, tmp_n, 60, 25);
		tmp_n += add;
		add(new JLabel("10%")).setBounds(30, tmp_n, 60, 25);
		tmp_n += add;
		add(new JLabel("0%")).setBounds(30, tmp_n, 60, 25);
		tmp_n += add;
		add(new JLabel("-10%")).setBounds(30, tmp_n, 60, 25);
		tmp_n += add;
		add(new JLabel("-20%")).setBounds(30, tmp_n, 60, 25);
